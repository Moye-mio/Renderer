...
//   - GetOrCreatePipeline(device) 按当前 desc 走 device.CreatePipeline 状态缓存
//   - Apply(device, cmd) 把属性表中的 uniform / texture 写入到对应绑定点（
//     仅记录到 cmd 的 SetUniform/SetTexture 命令；接入 spirv-cross 后会用
...
namespace TitusRHI
{
    ...
    public:
        ...
        // 由于 CreatePipeline 走状态缓存，重复调用零开销。
        ...
        // 仅 BindPipeline；属性 Apply 留给业务侧逐项 cmd.SetUniform/SetTexture
        // （或在反射就绪后自动）
