...
| `006_Dynamic_Diffuse_GI/` | Dynamic Diffuse GI（Sponza；Compute rayQuery 更新 probe） |
...
| `006_Dynamic_Diffuse_GI` | Dynamic Diffuse GI（Sponza；GBuffer + Compute rayQuery probe） | `vk` |
...
- Windows + Visual Studio 2019 或更新版本（打开根目录 `TitusGLRenderer.sln`），目标平台仅 **x64**。推荐使用 **v143** 工具集。
- 使用 Vulkan 后端时需安装 [Vulkan SDK](https://vulkan.lunarg.com/)，并确保 `VULKAN_SDK`（或旧版安装器写的 `VK_SDK_PATH`）已设置；缺失时构建会直接给出提示。不涉及 Vulkan 的工程（`Basic` / `RendererGL` 等）在未装 SDK 的机器上仍可单独构建。
