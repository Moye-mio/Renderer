# 000 — Forward / Deferred / Clustered Forward 对比结果

同一场景、同一套灯、同一套 BRDF 下三种着色算法的开销对比。截图取自 2026-08-16 20:30 的同一次运行，
期间只切换 ImGui 面板的 Shading Technique，相机与灯参数未动。
...
三种算法读同一份 `SharedShadingParams`，光源 UBO 布局也相同（`LightBlockData`，32016 B），
所以差异只来自光照的组织方式。
