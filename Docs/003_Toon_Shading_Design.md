...
保留：`Body` / `Dress` / `Hair` / `Face`。源 FBX 里 Body 是一张网格、按材质分成这三部分，导出 OBJ 时拆开。

跳过：`Face_Eye`（BlendShape 眼球）、`Brow`、`EffectMesh` / `EyeStar`（无贴图占位）。M1 脸上的眼睛来自 Face Diffuse，没有独立眼球网格。
