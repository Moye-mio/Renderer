...
| 命名空间 | 行号 | 职责 | 关键 API |
|---|---|---|---|
| `APP` | `:88` | 应用生命周期 + 后端/线程选择 | `SetBackend`(91)/`SetThreadingMode`(92)/`GetBackend`(93)/`GetThreadingMode`(94)/`ParseCommandLine`(98)；`InitApp`(103)/`UpdateApp`(105)/`ShutdownApp`(107)；`ShouldClose`(111)/`WaitIdle`(113)；`AddPass`(120)；`UploadGpuModel`(128)/`DestroyGpuModel`(129)/`GetGpuModelInternal`(135)；`RunUnitTests`(141) |
...
1. `ParseCommandLine`：解析 `--backend=gl|vk|null` 与 `--threading=direct|threaded|nonthreaded`；未指定则用默认（VK→Threaded、GL→Direct）。
2. `InitApp`：
   ...
   - `PassScheduler::SetDevice` + `InitAllPasses`。
3. `UpdateApp`：主循环每帧 `PassScheduler::DrawFrame`；`ShouldClose` 对 VK 自管窗口经 `IGDevice::IsWindowClosed` 问询。
